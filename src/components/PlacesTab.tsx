...
export function PlacesTab({
  ...
  useSimulatedActivity(
    places.map(p => p.id),
    handleSimBump,
    { minDelay: 12000, maxDelay: 30000, enabled: !loading }
  );
      ...
      <div className="flex gap-2 overflow-x-auto scrollbar-none">
        {filters.map(f => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-all duration-200 btn-press ${
              f === filter
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filteredPlaces.map((p, idx) => {
            ...
            const wasRecentlyBumped = recentBumpId === p.id;
            const isPopular = count >= 20;
                    ...
                    <div className="space-y-1.5">
                      {linkedEvents.map(ev => ev && (
                        <button
                          ...
                          className="flex items-center gap-2 w-full text-left px-3 py-2 rounded-xl bg-primary/5 hover:bg-primary/10 transition-colors btn-press"
                        >
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-border/40">
                    <div className="flex flex-col gap-0.5">
                      ...
                      {wasRecentlyBumped && !hasJoined && (
                        <span className="text-[11px] text-primary/70 animate-fade-in-subtle">Someone just joined</span>
                      )}
                    </div>
                    <Button
                      size="sm"
